...
func (c *deploymentVersion) GetRc() *components.ReplicationController {
	...
	containerConfig, ok := c.Containers["blackduck-authentication"]
	if !ok {
		return nil
	}
	...
	hubAuth := util.CreateReplicationControllerFromContainer(&horizonapi.ReplicationControllerConfig{Namespace: c.Namespace, Name: "authentication", Replicas: util.IntToInt32(1)}, "",
		[]*util.Container{hubAuthContainerConfig}, c.getAuthenticationVolumes(), initContainers,
		[]horizonapi.AffinityConfig{}, utils.GetVersionLabel("authentication", c.blackduck.Spec.Version), utils.GetLabel("authentication"), c.PullSecret)

	return hubAuth
}

// getAuthenticationVolumes will return the authentication volumes
func (c *deploymentVersion) getAuthenticationVolumes() []*components.Volume {
...
func (c *deploymentVersion) getAuthenticationVolumeMounts() []*horizonapi.VolumeMountConfig {
