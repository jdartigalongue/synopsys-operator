...
func (p *Updater) updateOpsSightCRD(opsSight *opssightapi.OpsSight, hubs []*opssightapi.Host) error {
	...
	opssight, err := p.opssightClient.SynopsysV1().OpsSights(p.config.CrdNamespace).Get(opssightName, metav1.GetOptions{})
	if err != nil {
	...
	_, err = p.opssightClient.SynopsysV1().OpsSights(p.config.CrdNamespace).Update(opssight)
	if err != nil {
